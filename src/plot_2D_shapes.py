...
import os
from numpy import ndarray, array, column_stack, concatenate, append
from numpy import rad2deg, deg2rad, sin, cos, tan, arccos, arcsin
from numpy import pi, sqrt, linspace, sign
import matplotlib.pyplot as plt
from matplotlib.pyplot import figure, fill, gca


def fine_tune_rve(rve_bounds):
    plt.axis("off")
    plt.xlim((rve_bounds[0], rve_bounds[2]))
    plt.ylim((rve_bounds[1], rve_bounds[3]))
    return
...
def plot_unit_cell(ruc_bbox,
    ...
    for (fibres_shape, inc_data) in inclusions_data.items():
        if fibres_shape.upper() == "CIRCLE":
            ...
            Plot2DShapes.plot_cshapes(xyt_ro_ri_alpha=inc_data,
                                      ...
                                      et=fibre_edge_thickness)
    # tune
    fine_tune_rve(rve_bounds=ruc_bbox)
    ...
    if h5_path is not None:
        pass
        # TODO make provisions for writing directly to h5 file from py_plot object
    return
